...
- **Session lifecycle** (`src/engine/session/`): `GameSession` is long-lived and holds the committed state; `GameSession.playTurn(action, rawInput)` is the sole public entry point. It parses input *before* opening a transaction (so malformed input never touches `StateManager`), runs the action against a single-use `ActionExecution` (formerly named `GameTurn` — renamed to avoid colliding with the LLM layer's own "Turn" vocabulary, see below; it's a one-action-per-transaction execution guard, not a player-facing turn), then commits or rolls back via `StateManager` (`src/engine/transaction/`), which owns the "one transaction open at a time" invariant. Exactly one action per turn is enforced today, deliberately — lifting that later for multi-action LLM turns should only change `ActionExecution`'s internals, not `GameSession`'s or `StateManager`'s public shape.
...
- **`ToolCallDispatcher`** (`src/gamemaster/llm/tool/`) is the only component in the LLM layer that ever touches `GameSession`/`ActionExecution`/`Action` — everything above it (conversation management, request assembly, the LLM client itself) only ever sees a `ToolResult`. Each dispatched tool call opens its own `GameSession.playTurn` (i.e. its own `ActionExecution`/transaction) — so one LLM turn (one player input through to narration) can span *multiple* engine-level `ActionExecution`s when the model calls more than one tool.
- **Turn vocabulary** (`src/gamemaster/llm/turn/`, `.../lifecycle/`): a single player-input-to-narration exchange is a "turn," modeled as three types at increasing altitude, each translated into the next rather than reused as one shape:
  ...
  - This intentionally mirrors `GameTurn`'s old name-collision lesson: pick vocabulary that's unique to the layer/altitude it lives at (`ActionExecution` vs. `TurnDraft`/`TurnLifecycle`) rather than reusing a word like "Turn" or "Request" for two different-grained concepts.
