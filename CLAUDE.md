...
- **Turn seam** (`src/engine/entity/turn/`, `state/turn/`, `session/OnTurnEffect.ts`): the engine has no clock — the passage of game time is a monotonic **turn counter** in committed state (`TurnCounterState.count`), advanced once per *successful* `playTurn` (by the private `GameSession.advanceTurn`). `TurnCounter` is a single concrete concept — `current()` reads the count, `increment()` advances it — implemented by the state-backed `DefaultTurnCounter` (so it survives save/load and stays deterministic). `OnTurnEffect.apply(context)` is the injected post-turn hook — turn-scoped systems (status effects, hunger, day/night) implement it and run after each successful action; none ship yet. There is deliberately **no** generic "tick"/`TickSource` abstraction spanning turns and time: an earlier `TickSource` (read seam) / `TurnCounter` (add `advance()`) split was collapsed because the two intended mechanisms don't actually share a write signature (a turn `increment()`s by one; in-world time would advance by variable amounts) and their reads mean different things (turn ordinal vs. time-of-day) — a shared supertype was speculative structure, not a real abstraction. **"turn"** is the engine's concrete per-action mechanism (`playTurn`, `TurnCounter`, `advanceTurn`, `OnTurnEffect`) — one turn per player action. **"clock"/time-of-day** is reserved for a *possible future second mechanism* that advances in-world time at story moments (a quest milestone jumping 9 AM → 9 PM), not per action; when built it gets its **own** separate `Clock`/`DefaultClock` types, state, and post-advance hook (e.g. `OnClockAdvance`) with **no shared supertype** with `TurnCounter`. Don't reintroduce a joint "tick" abstraction to unify them, and don't name the turn counter `clock`.
...
- **Turn vocabulary** (`src/gamemaster/llm/turn/`, `.../lifecycle/`): a single player-input-to-narration exchange is a "turn," modeled as three types at increasing altitude, each translated into the next rather than reused as one shape:
  ...
  - This intentionally mirrors the engine-side naming discipline: pick vocabulary that's unique to the layer/altitude it lives at (the engine owns "turn" at the `playTurn`/`TurnCounter`/`OnTurnEffect` grain; the LLM layer owns `TurnDraft`/`TurnLifecycle`) rather than reusing a word like "Turn" or "Request" for two different-grained concepts.
